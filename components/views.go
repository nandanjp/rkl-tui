...
func updateChoices(msg tea.Msg, m ViewsModel) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.KeyMsg:
		switch msg.String() {
		case "j", "down":
...
func updateChosen(msg tea.Msg, m ViewsModel) (tea.Model, tea.Cmd) {
	switch msg.(type) {
	case frameMsg:
		if !m.Loaded {
			m.Frames++
...
func choicesView(m ViewsModel) string {
	c := m.Choice
...
func chosenView(m ViewsModel) string {
	var msg string
